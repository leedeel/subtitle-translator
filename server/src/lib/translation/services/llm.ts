...
const baseOpenAICompatServices = Object.fromEntries(OPENAI_COMPAT_KEYS.map((k) => [k, makeOpenAICompat(k)])) as Record<
  ...
  try {
    ...
    if (!params.useRelay && error instanceof Error) {
      if (error instanceof TypeError && error.message.includes("Failed to fetch")) {
        throw new Error(
          "Network error (possibly CORS). Please enable 'API Relay' in API Settings. / 网络错误（可能是 CORS 限制），请在 API 设置中开启「中转 API」。"
        );
      }
      if (error.message.includes("[403]")) {
        throw new Error(
          "DeepSeek API returned 403 Forbidden. Please enable 'API Relay' in API Settings. / DeepSeek API 返回 403 禁止访问，请在 API 设置中开启「中转 API」。"
        );
      }
    }
    throw error;
...
export const gemini: TranslationService = async (params) => {
  ...
  const data = (await fetchJSON(
    `https://generativelanguage.googleapis.com/v1beta/models/${model || defaultConfigs.gemini.model!}:generateContent?key=${key}`,
    {
      method: "POST",
...
const NVIDIA_THINKING_RULES: Array<{
  pattern: RegExp;
  build: (on: boolean) => Record<string, unknown>;
}> = [
...
const buildNvidiaThinkingParams = (model: string, enableThinking: boolean): Record<string, unknown> =>
  NVIDIA_THINKING_RULES.find((r) => r.pattern.test(model))?.build(enableThinking) ?? {};
